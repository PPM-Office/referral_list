...
impl ZoneReportDailyMap {
    pub async fn generate_report(church_client: &mut ChurchClient, holly_config: holly::config::Config) -> anyhow::Result<Self> {
    ...
    pub async fn send_report_to_holly(stream: &mut TcpStream, church_client: &mut ChurchClient, holly_config: holly::config::Config) -> anyhow::Result<()> {
        ...
        for (zone_id, zone_report) in report.0.iter() {
            let chat_id = holly_config.zone_chats.get(zone_id)
                .ok_or_else(|| anyhow::anyhow!("Zone ID {zone_id} not found in holly config"))?;
            let vars = zone_report.get_vars_for_template();
            let message = template.replace("{area_avg_response_time}", &vars.get("area_avg_response_time").unwrap_or(&"".to_string()))
    ...
    pub async fn pretty_print_report(&self, env: &crate::env::Env) -> String {
        ...
        for (zone_id, zone_report) in self.0.iter() {
            if let Some(zone_name) = get_zone_name_from_id(env, *zone_id) {
                ...
                let vars = zone_report
                    .get_vars_for_template();

                let report: Vec<&str> = vars
                    .values()
                    .clone()
                    .map(|s| s.as_str())
                    .collect();

                output.push_str(&format!("Average Response time:\n{}\nUnattempted Contacted Referrals:\n{}\n", report[0], report[1]));
            }
        }
        output
    } 
